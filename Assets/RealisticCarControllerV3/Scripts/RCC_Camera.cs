...
//		transform.rotation = playerCar.transform.rotation * Quaternion.AngleAxis(10f, Vector3.right);
	...
	private void FPS(){
        ...
        transform.rotation = Quaternion.Lerp(transform.rotation, playerCar.transform.rotation * Quaternion.Euler(orbitY, orbitX, 0f), Time.deltaTime * 50f);
	...
	private void TPS(){
		...
		float xAngle = 0f;
		xAngle = (useOrbitInTPSCameraMode ? orbitY : 0f);

		float yAngle = 0f;
		yAngle = Mathf.SmoothDampAngle(transform.eulerAngles.y, playerCar.transform.eulerAngles.y + dir + (useOrbitInTPSCameraMode ? orbitX : 0f), ref yVelocity, 1f - TPSRotationDamping);
		...
		position += Quaternion.Euler(xAngle, yAngle, 0f) * (new Vector3(TPSOffset.x, TPSOffset.y, -TPSDistance + TPSOffset.z));
		position += Vector3.up * TPSHeight;

		// Look at the target
		transform.eulerAngles = new Vector3(TPSPitch + xAngle, yAngle, TPSYaw + zAngle);
